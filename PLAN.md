...
## Phase 12 — 暖粉色 UI/UX 工作台（本批）

- [x] 按 `product-design:index` 先做上下文与现状审视；保留节点、角色、关系、证据四条主线，不改数据模型和离线单文件约束。
- [x] 形成三条文字设计方向：编辑台、流程台、控制台；因 ImageGen 额度限制无法生成视觉稿，用户授权按文字方向直接实施。
- [x] 建立暖粉色浅色 / 深色令牌：画布、表面、边线、文字、状态色和阴影统一收口，移除工作台内冷蓝 / 白色硬编码的视觉漂移。
- [x] 把“核心能力”放回主路径：阶段选中态、节点页签、工作台类型、交付结果和候选入口使用一致的活动 / 悬停 / 焦点反馈。
- [x] 改善真实使用细节：表单焦点环、按钮反馈、卡片 hover、详情关闭态、减少运动支持，以及窄屏选中阶段自动滚入可见区。
- [x] 重新生成 `public/index.html`，保持 4,184 个插件、5,425 条 Skill 证据、96 条 Agent 能力、149 个工作位和 14 条关系图数据不变；未改变 benchmark、认证或运行时结论。
- [x] 完成桌面、约 390 CSS px 窄屏、Agent 联合筛选、三件事详情和研究方法页浏览器回归；控制台 warning/error 为 0。
- [x] 更新 `design-qa.md`，记录文字方向回退、前后截图、尺寸、状态和剩余边界。
- [ ] ImageGen 视觉稿、浅色系统主题的独立截图和正式用户可用性测试待后续额度 / 环境可用时补做；本批不把它们伪装成已完成证据。

### Phase 12 执行 TODO 与交付边界

- 视觉：暖粉色不是渐变装饰，颜色只服务于“现在在哪一步、下一步做什么、证据是否足够”。
