...
1. **它能帮你做什么**：优先呈现插件独有动作与对象；不把目录声明升级为实测能力。
2. **用之前要准备什么**：安装状态、认证策略、能力形态与未知条件；不把 Skill 加载写成外部连接成功。
3. **怎么确认它真的能用**：给出最小验证、可观察结果、未验证缺口与副作用边界。

插件工作台的 42 个首选候选使用人工精炼的中文能力摘要和最小验证。Agent 能力库的 96 条候选逐条保存独立的“它能帮你做什么／用之前要准备什么／怎么确认它真的能用”，并额外记录工作角色、覆盖方式和证据入口等级；其中 40 条保持为 `v1-40` benchmark cohort，32 条为首批角色扩展，24 条为本批工作流扩展。完整目录的其余长尾项目保留来源说明，并使用同一套保真边界；不把机器生成的通用节点句冒充独有能力。
...
- `agentic-skill`：为兼容现有页面 schema 保留的能力记录类型。当前共有 96 条，其中原始 40 条属于 `v1-40` benchmark cohort，新增 32 条用于首轮角色覆盖，24 条属于 `role-expansion-v2-2026-09` 工作流扩展；有原生 Skill 时键为 `repo + HEAD + Skill path/blob`，没有原生 Skill 时使用明确的 workflow slug。

`SKILL.md` 的原始路径数会被示例、fixture、复制目录和相同 blob 放大。因此页面同时给出 8,779 条路径、4,326 个仓库内唯一 blob、91 条冻结原生路径和 5 条工作流 / wrapper 候选，并明确这些数字都不是“已安装 Skill 数”。本批 24 条扩展全部绑定了冻结路径或明确的候选载体；其中 Vercel 的 5 条因许可证边界未确认只作参考，不进入可复制安装结论。
...
本版 96 条能力只使用前两级，`runtimeVerified` 继续固定为 0；24 条 v2 扩展全部停在 `static-reviewed`，不自动进入旧 benchmark。第一轮另有 4 条 `v1-40` 单文件 Skill 进入“执行器校准”：只把固定 blob 放进一次性工作区，让同一模型在 baseline / Skill 两个 arm 上各运行两次。它验证的是 runner，不是候选效果，因此不会把候选升级为 `controlled-smoke` 或 `task-benchmarked`。
...
工作台映射解决“某个能力在哪个节点做什么”；关系图解决“一个节点交付什么，下一节点凭什么接手”。两者不能互相替代。关系图使用 `workflowGraph` 保存 14 条稳定的模块边（`WR01`–`WR14`），每条边都必须给出：

- `fromModule` / `toModule` 与各自的 lead role；
- 输入产物和输出产物（`A00`–`A13`）；
- 人话交接说明与下一节点验收条件；
- 参与交接的 `candidateSlugs`，用于从关系卡反查能力详情。

关系类型只有三种：`feeds` 表示普通交接，`blocks` 表示前置技术块未过就不能继续，`feedback` 表示后置复盘把修正意见回写到前序节点。它们是研究和验收的关系，不是隐藏的安装依赖、调用拓扑或自动授权。每条新增能力还必须声明至少一个 `workflowRef`，否则只能进入项目级审计，不能进入工作台。

本批关系覆盖从 M00 事实摄取、M01 数据边界、M02 需求、M03 领域模型、M04 决策、M05 接口、M06 体验、M07 架构、M08 实现、M09 质量、M10 发布、M11 观测、M12 运营到 M13 复盘的完整闭环；`WR14` 将 M13 的修正反馈回 M00。6 条能力边再补充了共享 schema、可视化产物、缺陷诊断、性能度量、谱系修正与部署前置之间的局部关系。页面展示的是接力证据，不能把“有关系”误写成“已验证能运行”。
...
96 条去重能力形成 149 个 `capability × module × leadRole` 分配。页面默认展示该角色内的有序候选，目录的节点与角色筛选必须命中同一条分配，不能把 `primaryModule` 和角色标签拼成不存在的适配。本批为原先稀疏的 M01 Review、M06 总结、M08 可视化、M09 总结、M10 可视化与 M13 可视化补入专门或组合候选；M03 核心仍只有一个高置信候选，因此保留单一来源提示，不为凑数补位。组合覆盖、通用补位、需封装或只作参考仍显式标记，不伪装成完整覆盖。
