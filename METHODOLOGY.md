...
插件工作台的 42 个首选候选使用人工精炼的中文能力摘要和最小验证。Agent 能力库的 96 条候选逐条保存独立的“核心能力／使用条件／最低验证”，并额外记录工作角色、覆盖方式和证据入口等级；其中 40 条保持为 `v1-40` benchmark cohort，32 条为首批角色扩展，24 条为本批工作流扩展。完整目录的其余长尾项目保留来源说明，并使用同一套保真边界；不把机器生成的通用节点句冒充独有能力。
