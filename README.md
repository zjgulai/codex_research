...
插件工作台的 42 个候选沿用原有人工编辑层。Agent 能力库从 81 个仓库中选出 72 条去重能力，其中 67 条绑定冻结的 `SKILL.md`，5 条是工作流或 wrapper 候选；这些能力在 M00–M13 中形成 116 个角色工作位。直接 Skill、整包依赖、通用补位、组合覆盖与需封装工具分开标记，不为“每个节点看起来都有”伪造专用能力。

原始 40 条候选继续作为固定的 `v1-40` benchmark cohort；新增的 32 条能力不会自动污染既有计划。首轮从 cohort 中取 4 条单文件 Skill 做了 16 次 baseline / Skill A/B 校准。执行器通过了固定 blob、显式读取、结构化输出和工作区零变化检查，但关键词评分器没有通过校准，因此本轮仍是 0 条 `smoke-tested`、0 条 `task-benchmarked`、0 条默认栈晋级。页面会显示这次失败，而不是拿不可靠的 delta 做排行。
...
- 当前 remote catalog Skill 证据：5,420 条，目录时间标记为 2026-09-16T08:03:00.433482Z；本次构建另固定其 SHA-256。与上次构建相比，2 条证据从 `frozen-exact` 漂移为 `current-version-drift`，受影响插件缺口从 21 增至 25，未静默沿用旧结论。
...
- 81 个仓库包含 8,779 条 `SKILL.md` 路径、4,326 个仓库内唯一 blob；这些是审计漏斗，不是 4,326 个可安装 Skill。最终能力库保留 72 条去重能力：67 条原生路径、5 条工作流 / wrapper 候选，共 116 个节点角色分配。
...
生成器会验证 canonical ID 唯一性、总数、单一研究主节点、技能父子关系、Agentic-Tools 列表成员与冻结 SHA、72 条能力、40 条 benchmark cohort、14 × 4 角色覆盖与连续名次、HTML CSP、压缩数据的大小与 SHA-256、内嵌数据一致性和敏感路径泄漏。
